    ...
    do {
      if (check(secondConn, curConn.id, traverseElement)) {
        ...
        traverseElement.children = [
          stopAt(firstConn, curConn.id, { type: "empty" }),
          stopAt(secondConn, curConn.id, { type: "empty" }),
        ];
  ...
  if (item) {
    const temp = {
      ...
      connections: elements
        ...
        .map((edge) => {
          const edgeTarget = elements.filter(
            (el) => isNode(el) && el.id === edge.target
          );
...
function generateScript(traverseItem, codeData, tabs = "") {
  ...
  if (traverseItem.type === "assign") {
    ...
    traverseItem.type === "if" &&
    traverseItem.children &&
    traverseItem.children.length === 2
  ) {
    output =
      tabs +
      "if (" +
    ...
    if (
      traverseItem.children.length > 1 &&
      traverseItem.children[1].type !== "empty"
    ) {
      output += tabs + "} else {\r\n";
      output += generateScript(
        traverseItem.children[1],
        codeData,
        tabs + "   "
      );
    }
...
const CodeDisplay = ({ nodes, codeData }) => {
  ...
  return (
    <>
      <div className="row">
        <div className="col pt-3 code-disply-cont">
          <button className="btn btn-primary code-disply-save-btn" >
            Save
          </button>
          <button className="btn btn-primary code-disply-deploy-btn" >
            Deploy
          </button>
        </div>
      </div>
